...
SELECT COUNT(*) AS COUNTT, BILLING_COUNTRY FROM INVOICE
...
SELECT DISTINCT TOTAL FROM INVOICE A WHERE 3 >=
(SELECT count(distinct TOTAL) from INVOICE b WHERE a.TOTAL <= b.TOTAL)
...
-- see the schema to join the table

SELECT DISTINCT email AS Email,first_name AS FirstName, last_name AS LastName, genre.name AS GEN_Name
FROM customer INNER JOIN invoice 
...
select artist.artist_id,artist.name as composer,COUNT(artist.artist_id) AS number_of_songs
